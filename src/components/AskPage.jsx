...
export default function AskPage() {
  ...
  const addComment = (questionIndex) => {
    const updatedQuestions = [...questions];
        ...
        <div>
          ...
          {questions.map((q, questionIndex) => (
            <QuestionContainer key={questionIndex}>
              <h3>
                {q.title}
                {editedIndexes.includes(questionIndex) && " (edited)"}
              </h3>
              <p>{q.body}</p>
              <EditButton onClick={() => editQuestion(questionIndex)}>
                Edit
              </EditButton>
              ...
              />
              <BlueButton
                onClick={() => addComment(questionIndex)}
                disabled={!comment}
              >
                Add Comment
              </BlueButton>
